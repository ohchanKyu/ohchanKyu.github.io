...
상관없이 브라우저에 데이터가 저장되므로 바로 key와 value값으로 갱신된 값을 저장하고 가져올 수 있다.   
...
- localStorage  
1. 사용자 세션 데이터를 유지한다.  
2. 브라우저를 닫았다가 다시 열어도 지속된다.  
3. 한 브라우저 창에 탭을 여러개 열어도 공유 가능하다.  
4. 삭제할때까지 데이터가 지속된다.  

이를 요약하면 사용자가 브라우저 창을 닫았을 때 데이터가 삭제되지 않으며,
만료 날짜없이 해당 데이터를 저장하고 JS 코드를 통해 사용가능하다.

- sessionStorage
1. 브라우저의 세션 기간동안에만 사용가능하다.
2. 탭이나 창을 닫을때는 삭제된다.

따라서 이는 새로운 창끼리 공유가 되지 않으며, 해당 브라우저 창을 닫는다면 데이터가 삭제된다.  

### Project에서 localStorage의 사용 이유
위의 차이점을 보았듯이 accessToken은 사용자가 로그인 후 인증의 단계에서 사용하게 된다.  
따라서 로그인을 한 후에 창을 닫아도 해당 창을 다시 열었을때 로그인이 되어있는 상태가  
사용자 입장에서 사용자 경험 측면에서 편할 수 밖에 없다. 따라서 sessionStorage보다는  
localStorage를 사용하는 것이 사용자 경험의 개선면에서 유리하다.

### localStorage의 취약점
하지만 너무 오랫동안 localStorage에 접근 가능한 accessToken을 저장하는 것은 좋지 않다.  
따라서 accessToken을 주기적으로 갱신해줄 필요는 있다. 이를 위해 refreshToken이 존재하는 것이다.  
이는 JWT 인증 방식에 대한 아래 포스팅을 참고해주면 된다. 이처럼 localStorage는 웹 브라우저의   
저장소로 유용하게 쓰일 수 있지만, 개발자 도구를 통해 쉽게 해당 정보를 접근하거나 공유가능하다.  
따라서 보안에 취약하다는 치명적인 단점이 있다. 따라서 위의 JWT 방식처럼 해당 정보를 주기적으로  
