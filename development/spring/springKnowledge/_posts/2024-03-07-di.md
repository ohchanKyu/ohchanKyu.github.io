...
## 빈을 주입하는 방법
생성자 주입 방식은 수정자, 필드 주입방식과 빈을 주입하는 순서가 다르다.  
생성자 주입은 객체를생성 시점에 필요한 빈을 찾아 주입한다. 
순환참조시 서로가 참조하는 객체인경우엔 참조할 객체가 생성되지 않은 상태에 
그 객체의 빈을 찾을 수 없으므로 오류가 발생한다.

그러나 수정자 주입과 필드 주입에서는 빈이 먼저 생성된다. 
빈이 먼저 생성된 후에 필드에 주입하거나, 빈 객체의 수정자를 호출하여 주입한다.
